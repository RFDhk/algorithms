...
public class Point implements Comparable<Point> {
    ...
    public double slopeTo(Point that) {
        ...
        if (that.getY() == this.getY() && that.getX() == this.getX()) return Double.NEGATIVE_INFINITY;

        else if (that.getX() == this.getX()) return Double.POSITIVE_INFINITY;

        else if (that.getY() == this.getY()) return +0.0d;

        else return (double) (that.getY() - this.getY()) / (double) (that.getX() - this.getX());
    }
